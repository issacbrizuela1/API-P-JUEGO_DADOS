...
class END_SALA:
    ...
    def add(self, data: MODEL_USUARIO):
        query = f"INSERT INTO sala (clave, password, configsala, owner, ganador, activo)\
            VALUES({uuid.uuid4()}, {password}, {configsala}, {owner}, {ganador}, 1)"
        response = self.conn.insert(self.conn, query)
        return response

    def getAll(self):
        response = self.conn.select(self.conn, 'select * from sala')
        return response
